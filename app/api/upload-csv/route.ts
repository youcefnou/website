...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    let dedupedRows = Array.from(dedupedRowsMap.values());
    ...
    if (importMode === 'existing') {
      ...
      // Filter out rows whose model_name matches the product's own name
      // This prevents the product name from appearing among its own variants
      const productNameLower = existingProduct.name.trim().toLowerCase();
      dedupedRows = dedupedRows.filter((row) => {
        const modelLower = String(row.model_name || '').trim().toLowerCase();
        return modelLower !== productNameLower;
      });
    } else {
